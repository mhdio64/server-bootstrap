# Context Pack Manifest

Generated: 2026-09-02

Canonical repository: `mhdio64/server-bootstrap`

Files:
- `.cursor/rules/00-governance.mdc`
- `.cursor/rules/10-ansible-quality.mdc`
- `.cursor/rules/20-security-safety.mdc`
- `.cursor/rules/30-tests-ci.mdc`
- `.cursor/skills/implement-approved-phase/SKILL.md`
- `AGENTS.md`
- `README.md`
- `docs/00-CONTEXT-INDEX.md`
- `docs/01-PROJECT-SPEC.md`
- `docs/02-ARCHITECTURE.md`
- `docs/03-SECURITY-SAFETY.md`
- `docs/04-IMPLEMENTATION-PLAN.md`
- `docs/05-TESTING-RELEASE.md`
- `docs/06-CURSOR-WORKFLOW.md`
- `docs/07-DECISIONS.md`
- `docs/08-RISKS-ASSUMPTIONS.md`
- `docs/09-VERSION-SOURCES.md`
- `docs/10-PHASE-STATUS.md`
- `docs/11-INTERVIEW-DECISIONS.md`
- `docs/12-GRAPHIFY.md`
